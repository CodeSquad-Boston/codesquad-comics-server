...
const getAllBooks = async (req, res, next) => {
    try {
      //Direction: For the getAllBooks route, after the await keyword, tell the Book model to use the .find() method with an empty object as the parameter AND chain that to the .then() method with a parameter of books. 
      await Book.find({}).then((books) => //have students add this line of code
      res.status(200).json({
        success: { message: "Found all books!" },
        data: books, //Within the json component, change the data value to books.
        statusCode: 200,
      })
    );
    } catch (err) {
