...
//Direction: within the arrow function's object, stage a try/catch statement. Remember that the await keyword should trigger the try functions code block.
...
//Direction: within the arrow function's object, stage a try/catch statement. 
    const { id } = req.params;
    //test 
    // (?) const {id} = uuid();
    try {
        res
            ...
            .json({ success: { message: "Found the book!" },
             data: {foundBook: 
                //replace id with... data.js object with UUID(?)
                id}, statusCode: 200 });
    /*
